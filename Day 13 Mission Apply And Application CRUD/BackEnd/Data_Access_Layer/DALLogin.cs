        ...
        public string LoginUserProfileUpdate(UserDetail userDetail)
        {
            ...
            try
            {
                ...
                using (var transaction = _cIDbContext.Database.BeginTransaction())
                {
                    try
                    {
                        // Validate userDetail object
                        if (userDetail == null)
                        {
                            throw new ArgumentException("UserDetail object is null.");
                        }
                        if (userDetail.UserId <= 0)
                        {
                            throw new ArgumentException("Invalid UserId.");
                        }
                        if (string.IsNullOrWhiteSpace(userDetail.FirstName))
                        {
                            throw new ArgumentException("FirstName is required.");
                        }
                        if (string.IsNullOrWhiteSpace(userDetail.LastName))
                        {
                            throw new ArgumentException("LastName is required.");
                        }
                        if (string.IsNullOrWhiteSpace(userDetail.EmailAddress))
                        {
                        ...
                        // Save changes to the UserDetail table
                        _cIDbContext.SaveChanges();
                    ...
                    catch (Exception ex)
                    {
                        ...
                        result = "An error occurred while updating the user profile. Please try again.";
                        throw;  // Optionally rethrow the exception if you need further handling
                    }
                }
            }
            catch (Exception ex)
            {
                // Log the outer exception if transaction initiation fails
                Console.WriteLine(ex.InnerException?.Message ?? ex.Message);
                result = "An error occurred while updating the user profile. Please try again.";
            }
            return result;
